...
-- ADD COLUMN IF NOT EXISTS não corrige o DEFAULT quando a coluna já veio do
-- serviço antigo. Sem estes ajustes, INSERTs novos podem falhar com 23502 em
-- bancos onde verified, featured, created_at ou updated_at são NOT NULL e não
-- possuem valor padrão. A operação é idempotente e não altera registros atuais.
...
-- Versões antigas do serviço de grupos criavam um CHECK fechado para category.
-- A tela passou a oferecer "Free lances", mas a regra legada continuava
-- rejeitando o INSERT com 23514. Substituímos qualquer CHECK que mencione a
-- coluna e mantemos os rótulos antigos apenas para compatibilidade dos registros
-- já publicados; eles não voltam a aparecer no formulário.
DO $$
DECLARE
  item RECORD;
BEGIN
