...
class FakePool {
  constructor() {
    ...
    this.groups = [];
    this.jobs = [];
    this.rejectGroupCategory = false;
  }
  ...
  async query(sql, values = []) {
    ...
    if (text.startsWith('SELECT tablename FROM pg_catalog.pg_tables')) {
      return result((values[0] || []).map((tablename) => ({ tablename })));
    }
    if (text.startsWith('SELECT pg_get_constraintdef(oid) definition FROM pg_constraint')) {
      return result([{ definition: "CHECK ((category = ANY (ARRAY['Empregos'::text, 'Vagas'::text, 'Free lances'::text])))" }]);
    }
    if (text.startsWith('SELECT column_name FROM information_schema.columns')) {
      return result(['name', 'slug', 'description', 'invite_url', 'category', 'state', 'city', 'group_type', 'status']
    ...
    if (text.startsWith('INSERT INTO gg_groups')) {
      if (this.rejectGroupCategory) {
        const error = new Error('new row violates check constraint');
        error.code = '23514';
        error.constraint = 'gg_groups_category_check';
        throw error;
      }
      const group = {
...
async function main() {
  ...
  registerCommunityRoutes({
    ...
    config: {
      ...
      APP_VERSION: '13.0.1',
      LOCATION_FETCH: async () => ({
        ok: true,
        status: 200,
        json: async () => [{ nome: 'São Paulo' }, { nome: 'Campinas' }, { nome: 'Osasco' }],
      }),
    },
    helpers: {
      escapeHtml,
  ...
  try {
    let response = await request('/health/communities');
    assert.equal(response.status, 200);
    const healthBody = await response.json();
    assert.equal(healthBody.status, 'ok');
    assert.equal(healthBody.version, '13.0.1');
    assert.equal(healthBody.checks.category_free_lances, 'ok');
    assert.equal(healthBody.checks.legacy_required_columns, 'ok');

    response = await request('/api/public/localidades/estados/SP/municipios');
    assert.equal(response.status, 200);
    let locationBody = await response.json();
    assert.equal(locationBody.completo, true);
    assert.deepEqual(locationBody.municipios, ['Campinas', 'Osasco', 'São Paulo']);
    response = await request('/api/public/localidades/estados/XX/municipios');
    assert.equal(response.status, 400);

    response = await request('/cadastro');
    assert.equal(response.status, 200);
    const registrationHtml = await response.text();
    assert.match(registrationHtml, /data-location-state/);
    assert.doesNotMatch(registrationHtml, />UF</);
    let csrf = csrfFrom(registrationHtml);
    response = await request('/cadastro', {
      method: 'POST',
    ...
    pool.rejectGroupCategory = true;
    const rejectedGroup = new FormData();
    Object.entries({
      csrf_token: csrf, website: '', preview_image_url: '',
      invite_url: 'https://chat.whatsapp.com/ZyxwVutsrq98765',
      name: 'Free lances em Campinas',
      description: 'Grupo para oportunidades de trabalho autônomo e prestação de serviços na região de Campinas.',
      rules: '', category: 'Free lances', state: 'SP', city: 'Campinas', region: '', group_type: 'emprego',
    }).forEach(([key, value]) => rejectedGroup.append(key, value));
    response = await request('/minha-conta/grupos/novo', { method: 'POST', body: rejectedGroup });
    assert.equal(response.status, 400);
    assert.match(await response.text(), /categoria selecionada ainda não está habilitada no banco/i);
    pool.rejectGroupCategory = false;
