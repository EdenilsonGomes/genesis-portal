...
const GROUP_CATEGORIES = [
  ...
  'Primeiro emprego', 'Estágio e jovem aprendiz', 'PCD', 'Trabalho temporário',
...
const accountSchema = z.object({
  ...
  estado: z.string().trim().max(2).optional().default('SP'),
...
function registerCommunityRoutes({ app, pool, config, helpers }) {
  const { SITE_URL, PANEL_URL, AUTH_SECRET, SESSION_DAYS, PUBLICATIONS_WEBHOOK_URL, PUBLICATIONS_WEBHOOK_TOKEN } = config;
  const { escapeHtml, slugify, truncate, metaPage, header, footer, organizationSchema, vacancyUrl, listVacancies, formatMoney } = helpers;
  ...
  function flash(req) {
    if (req.query.ok) return `<div class="form-alert success">${escapeHtml(String(req.query.ok))}</div>`;
    if (req.query.erro) return `<div class="form-alert error">${escapeHtml(String(req.query.erro))}</div>`;
    return '';
  }
  ...
  function pageHeader(account) { return header({ account, accountAction: `<a class="btn btn-ghost" href="${account ? '/minha-conta' : '/entrar'}">${account ? 'Minha conta' : 'Entrar'}</a>` }); }
  async function notify(event, data) {
    if (!PUBLICATIONS_WEBHOOK_URL) return;
    try {
      await fetch(PUBLICATIONS_WEBHOOK_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: PUBLICATIONS_WEBHOOK_TOKEN, event, source: 'genesis-portal', data }), signal: AbortSignal.timeout(8000) });
  ...
  function groupImageUrl(group) {
    ...
    return group.image_url || `${SITE_URL}/assets/genesis-mark.svg`;
  }
  function authPage(req, res, { title, subtitle, content }) {
    return metaPage({ title, description: subtitle, canonical: `${SITE_URL}${req.path}`, image: `${SITE_URL}/assets/genesis-social.svg`, bodyClass: 'light-page', robots: 'noindex,nofollow', nonce: res.locals.cspNonce, structuredData: [], content: `${pageHeader(null)}<main id="conteudo" class="auth-main"><div class="container auth-grid"><section class="auth-message"><span class="eyebrow">Portal Gênesis</span><h1>${escapeHtml(title)}</h1><p>${escapeHtml(subtitle)}</p><ul class="auth-benefits"><li>Publique grupos e vagas</li><li>Acompanhe análise e desempenho</li><li>Conheça as soluções da Gênesis</li></ul></section><section class="auth-card">${content}</section></div></main>${footer()}` });
  }

  function registrationPage(req, res, { errors = {}, values = {} } = {}) {
    const token = csrf(req, res);
    const content = `${flash(req)}<h2>Criar conta gratuita</h2><p class="form-intro">Use uma conta simples para gerenciar suas publicações e acompanhar resultados.</p>
      <form method="post" action="/cadastro" class="stack-form" novalidate>
        ...
        <div class="form-grid two"><label>Cidade<input name="cidade" value="${escapeHtml(values.cidade || '')}" autocomplete="address-level2"></label><label>UF<input name="estado" maxlength="2" value="${escapeHtml(values.estado || 'SP')}" autocomplete="address-level1"></label></div>
    ...
    return authPage(req, res, { title: 'Publique e acompanhe oportunidades', subtitle: 'Uma área própria para recrutadores e empresas gerenciarem grupos, vagas e métricas.', content });
  }

  function loginPage(req, res, { errors = {}, values = {} } = {}) {
    const token = csrf(req, res);
    const retorno = cleanRedirect(req.query.retorno || values.retorno || '/minha-conta');
    const content = `${flash(req)}<h2>Entrar na sua conta</h2><p class="form-intro">Gerencie as publicações enviadas ao portal.</p>
    ...
    return authPage(req, res, { title: 'Bem-vindo de volta', subtitle: 'Entre com seu e-mail e senha. O painel operacional dos clientes continua separado.', content });
  }

  function groupCard(group, compact = false) {
    const location = [group.region, group.city, group.state].filter(Boolean).join(' · ');
    return `<article class="group-card${group.featured ? ' featured' : ''}"><a class="group-card-image" href="/grupo/${escapeHtml(group.slug)}"><img src="${escapeHtml(groupImageUrl(group))}" alt="Foto do grupo ${escapeHtml(group.name)}" width="360" height="220" loading="lazy">${group.featured ? '<span class="group-featured">Destaque</span>' : ''}</a><div class="group-card-body"><div class="group-card-tags"><span>${escapeHtml(group.category)}</span>${group.verified ? '<span class="verified-chip">✓ Verificado</span>' : ''}</div><h3><a href="/grupo/${escapeHtml(group.slug)}">${escapeHtml(group.name)}</a></h3><p class="group-location">⌖ ${escapeHtml(location || 'Brasil')}</p><p>${escapeHtml(truncate(group.description, compact ? 110 : 155))}</p><div class="group-card-footer"><span><b>${Number(group.click_count || 0).toLocaleString('pt-BR')}</b> acessos</span><a href="/grupo/${escapeHtml(group.slug)}">Ver grupo →</a></div></div></article>`;
  }
  ...
  async function directoryPage(req, res, next, fixed = {}) {
    try {
      ...
      const content = `${pageHeader(account)}<main id="conteudo" class="groups-directory"><section class="directory-hero"><div class="container"><div class="directory-title-row"><div><span class="eyebrow">Comunidades profissionais</span><h1>${escapeHtml(title)}</h1><p>${escapeHtml(description)}</p></div><a class="btn btn-accent" href="${account ? '/minha-conta/grupos/novo' : '/cadastro'}">Cadastrar meu grupo</a></div><form class="directory-search" action="/grupos" method="get"><label><span>Buscar</span><input name="q" value="${escapeHtml(q)}" placeholder="Cargo, profissão, cidade ou grupo"></label><label><span>Categoria</span><select name="categoria"><option value="">Todas</option>${GROUP_CATEGORIES.map((item) => `<option ${item === category ? 'selected' : ''}>${escapeHtml(item)}</option>`).join('')}</select></label><label><span>Cidade</span><input name="cidade" value="${escapeHtml(city)}" placeholder="São Paulo"></label><button class="btn btn-primary" type="submit">Buscar grupos</button></form><div class="category-scroller"><a href="/grupos">Todos</a>${GROUP_CATEGORIES.slice(0, 10).map((item) => `<a href="/grupos/categoria/${slugify(item)}">${escapeHtml(item)}</a>`).join('')}</div></div></section><section class="container directory-results"><div class="results-head"><div><h2>${data.total.toLocaleString('pt-BR')} grupo${data.total === 1 ? '' : 's'}</h2><span>Conteúdo revisado antes da publicação</span></div><a href="/vagas">Ver vagas abertas →</a></div>${data.groups.length ? `<div class="group-grid">${data.groups.map((group) => groupCard(group)).join('')}</div>${pages > 1 ? `<nav class="pagination">${pagination}</nav>` : ''}` : `<div class="empty-state"><h3>Nenhum grupo encontrado</h3><p>Tente outra busca ou cadastre uma comunidade.</p><a class="btn btn-primary" href="${account ? '/minha-conta/grupos/novo' : '/cadastro'}">Cadastrar grupo</a></div>`}</section><section class="container directory-business-cta"><div><span>Você recruta ou representa uma empresa?</span><h2>Transforme divulgação em um processo seletivo organizado.</h2><p>A Gênesis atende candidatos, realiza triagem, organiza documentos e acompanha entrevistas.</p></div><a class="btn btn-accent btn-lg" href="/anunciar-vaga">Conhecer a Gênesis</a></section></main>${footer()}`;
      const searchPage = Boolean(q || (!fixed.category && req.query.categoria) || (!fixed.city && req.query.cidade));
      return res.send(metaPage({ title, description, canonical, image: `${SITE_URL}/assets/genesis-social.svg`, bodyClass: 'light-page', nonce: res.locals.cspNonce, robots: searchPage ? 'noindex,follow,max-image-preview:large' : 'index,follow,max-image-preview:large', structuredData: [organizationSchema(), collectionSchema(data.groups, canonical, title)], content }));
    } catch (error) { return next(error); }
  }
  ...
  async function processImage(file, remoteUrl) {
    ...
    if (!buffer && remoteUrl) {
      try {
        ...
        const response = await fetch(url, { redirect: 'follow', signal: AbortSignal.timeout(7000) });
        const length = Number(response.headers.get('content-length') || 0);
        if (!response.ok || length > 5 * 1024 * 1024) return null;
  ...
  app.post('/minha-conta/grupos/novo', requireAccount, publicationLimiter, upload.single('image'), async (req, res, next) => {
    try {
      assertCsrf(req);
      const account = await currentAccount(req);
  ...
  app.post('/minha-conta/grupos/:id/editar', requireAccount, publicationLimiter, upload.single('image'), async (req, res, next) => {
    try {
      assertCsrf(req);
      const account = await currentAccount(req);
