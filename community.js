...
function registerCommunityRoutes({ app, pool, config, helpers }) {
  const { SITE_URL, PANEL_URL, AUTH_SECRET, SESSION_DAYS, PUBLICATIONS_WEBHOOK_URL, PUBLICATIONS_WEBHOOK_TOKEN, PORTAL_BRAND_NAME = 'Vagas & Grupos', PORTAL_BRAND_TAGLINE = 'Emprego, carreira e networking', APP_VERSION = '12.1.1' } = config;
  ...
  app.post('/minha-conta/grupos/novo', requireAccount, publicationLimiter, parseGroupImage, async (req, res, next) => {
    try {
      ...
      if (error?.code === '23514' || error?.code === '23502') { res.status(400); return groupFormPage(req, res, next, { errors: { geral: 'Confira os campos obrigatórios e tente novamente.' }, values: req.body || {} }); }
      return next(error);
    }
  });
