...
function registerCommunityRoutes({ app, pool, config, helpers }) {
  const { SITE_URL, PANEL_URL, AUTH_SECRET, SESSION_DAYS, PUBLICATIONS_WEBHOOK_URL, PUBLICATIONS_WEBHOOK_TOKEN, PORTAL_BRAND_NAME = 'Vagas & Grupos', PORTAL_BRAND_TAGLINE = 'Emprego, carreira e networking', APP_VERSION = '13.0.1', LOCATION_FETCH = globalThis.fetch } = config;
  ...
  async function accountDashboard(req, res, next) {
    try {
      const account = await currentAccount(req);
      const companyPortalPromise = account.tipo === 'EMPRESA'
        ? pool.query('SELECT slug,status,onboarding_step FROM portal_empresas WHERE owner_account_id=$1 LIMIT 1', [account.id]).catch((error) => {
      ...
      const companyPortalBanner = account.tipo === 'EMPRESA' ? `<section class="account-company-portal"><div><span>${companyPortal ? 'Portal da empresa' : 'Novo benefício para empresas'}</span><h2>${companyPortal ? 'Seu portal de carreiras está pronto para ser gerenciado.' : 'Crie um Trabalhe Conosco com a marca da sua empresa.'}</h2><p>${companyPortal ? 'Publique vagas diretamente, acompanhe acessos e compartilhe um único link com candidatos.' : 'Configure sua página, identidade e primeira vaga em poucos minutos.'}</p></div><a class="btn btn-primary" href="${companyPortal ? '/meu-portal' : '/portal-para-empresas'}">${companyPortal ? 'Gerenciar meu portal' : 'Criar portal grátis'}</a></section>` : '';
      const token = csrf(req, res);
