...
function registerCommunityRoutes({ app, pool, config, helpers }) {
  const { SITE_URL, PANEL_URL, AUTH_SECRET, SESSION_DAYS, PUBLICATIONS_WEBHOOK_URL, PUBLICATIONS_WEBHOOK_TOKEN, PORTAL_BRAND_NAME = 'Vagas & Grupos', PORTAL_BRAND_TAGLINE = 'Emprego, carreira e networking' } = config;
  ...
  async function groupFormPage(req, res, next, { group = null, errors = {}, values = {} } = {}) {
    try {
      ...
      return res.send(metaPage({ title, description: 'Cadastre gratuitamente um grupo de emprego, carreira ou networking.', canonical: `${SITE_URL}${req.path}`, image: `${SITE_URL}/assets/vagas-grupos-social.svg`, bodyClass: 'light-page directory-site', nonce: res.locals.cspNonce, robots: 'noindex,nofollow', siteName: PORTAL_BRAND_NAME, titleSuffix: PORTAL_BRAND_NAME, favicon: '/assets/vagas-grupos-mark.svg', themeColor: '#19ad5b', structuredData: [], content }));
    } catch (error) { return next(error); }
  }
  ...
  async function jobFormPage(req, res, next, { errors = {}, values = {} } = {}) {
    try {
      ...
      return res.send(metaPage({ title: 'Enviar vaga', description: 'Envie uma vaga para análise e publicação no Vagas & Grupos.', canonical: `${SITE_URL}/minha-conta/vagas/nova`, image: `${SITE_URL}/assets/vagas-grupos-social.svg`, bodyClass: 'light-page directory-site', nonce: res.locals.cspNonce, robots: 'noindex,nofollow', siteName: PORTAL_BRAND_NAME, titleSuffix: PORTAL_BRAND_NAME, favicon: '/assets/vagas-grupos-mark.svg', themeColor: '#19ad5b', structuredData: [], content }));
    } catch (error) { return next(error); }
  }
  ...
  app.get('/health/communities', async (_req, res) => {
    ...
    try {
      ...
      return res.status(missing.length ? 503 : 200).json({
        ...
        module: 'vagas-grupos',
        database: missing.length ? 'migration_required' : 'ok',
        missing_tables: missing,
        version: '12.1.0',
      });
    } catch (error) {
      console.error('[HEALTH_COMMUNITIES]', error);
      return res.status(503).json({ status: 'error', module: 'vagas-grupos', database: 'unavailable', version: '12.1.0' });
    }
  });
