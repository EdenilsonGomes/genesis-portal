...
function registerCommunityRoutes({ app, pool, config, helpers }) {
  const { SITE_URL, PANEL_URL, AUTH_SECRET, SESSION_DAYS, PUBLICATIONS_WEBHOOK_URL, PUBLICATIONS_WEBHOOK_TOKEN, PORTAL_BRAND_NAME = 'Gênesis Vagas', PORTAL_BRAND_TAGLINE = 'Pessoas, oportunidades e tecnologia', APP_VERSION = '13.0.1', LOCATION_FETCH = globalThis.fetch } = config;
  ...
  async function groupDetailPage(req, res, next) {
    try {
      ...
      return res.send(metaPage({ title, description, canonical, image: groupImageUrl(group), bodyClass: 'light-page directory-site', nonce: res.locals.cspNonce, siteName: PORTAL_BRAND_NAME, titleSuffix: PORTAL_BRAND_NAME, favicon: '/assets/vagas-grupos-mark.svg', themeColor: '#0B1F2A', structuredData: [portalOrganizationSchema(), schema], content }));
    } catch (error) { return next(error); }
  }
  ...
  async function accountDashboard(req, res, next) {
    try {
      const account = await currentAccount(req);
      const companyPortalPromise = pool.query('SELECT slug,status,onboarding_step,nome_publico FROM portal_empresas WHERE owner_account_id=$1 LIMIT 1', [account.id]).catch((error) => {
        if (error && error.code === '42P01') return { rows: [], rowCount: 0 };
        throw error;
      });
      const [groups, jobs, companyPortalResult] = await Promise.all([
  ...
  async function jobFormPage(req, res, next, { errors = {}, values = {} } = {}) {
    try {
      ...
      return res.send(metaPage({ title: 'Enviar vaga', description: 'Envie uma vaga para análise e publicação no Gênesis Vagas.', canonical: `${SITE_URL}/minha-conta/vagas/nova`, image: `${SITE_URL}/assets/vagas-grupos-social.png`, bodyClass: 'light-page directory-site', nonce: res.locals.cspNonce, robots: 'noindex,nofollow', siteName: PORTAL_BRAND_NAME, titleSuffix: PORTAL_BRAND_NAME, favicon: '/assets/vagas-grupos-mark.svg', themeColor: '#0B1F2A', structuredData: [], content }));
    } catch (error) { return next(error); }
  }
