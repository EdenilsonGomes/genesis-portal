...
const { registerCompanyPortalRoutes } = require('./company-portals');
const { registerDivulgacaoTracking } = require('./divulgacao-tracking');
const {
  BRAZIL_STATES,
  BRAZIL_STATE_CODES,
  MAIN_STATE_CODES,
...
function publicStateOptions(selected = '', { allowBlank = false } = {}) {
  const active = normalizeStateCode(selected);
  const render = (states) => states.map((state) => `<option value="${state.code}" ${state.code === active ? 'selected' : ''}>${escapeHtml(state.name)}</option>`).join('');
  ...
  return `${allowBlank ? '<option value="">Todos os estados</option>' : '<option value="">Selecione o estado</option>'}<optgroup label="Estados mais procurados">${render(popular)}</optgroup><optgroup label="Demais estados">${render(remaining)}</optgroup>`;
}

function publicCityOptions(state, selected = '', { allowBlank = false } = {}) {
  const active = String(selected || '').trim();
  const cities = mainCities(state);
  const known = new Set(cities.map((city) => city.toLocaleLowerCase('pt-BR')));
  const preserved = active && !known.has(active.toLocaleLowerCase('pt-BR')) ? `<option value="${escapeHtml(active)}" selected>${escapeHtml(active)}</option>` : '';
  return `<option value="">${allowBlank ? 'Todas as cidades' : 'Selecione a cidade'}</option>${preserved}${cities.map((city) => `<option value="${escapeHtml(city)}" ${city.toLocaleLowerCase('pt-BR') === active.toLocaleLowerCase('pt-BR') ? 'selected' : ''}>${escapeHtml(city)}</option>`).join('')}`;
}
...
function candidateApplication(vacancy) {
  ...
  if (channel === 'URL_EXTERNA' && vacancy.candidatura_url) {
    const externalUrl = String(vacancy.candidatura_url || '');
    const isWhatsAppDirect = /https?:\/\/(?:wa\.me|api\.whatsapp\.com)\//i.test(externalUrl);
    return {
      url: externalUrl,
      label: isWhatsAppDirect ? 'Candidatar-se pelo WhatsApp da empresa' : 'Candidatar-se no site da empresa',
      type: isWhatsAppDirect ? 'whatsapp-external' : 'external',
    };
  }
...
function commercialWhatsAppUrl() {
  const message = encodeURIComponent('Olá! Quero conhecer a Gênesis e organizar melhor as contratações da minha empresa.');
...
function upgradeLocationMarkup(html) {
  return String(html || '')
    .replace('Ex.: vagas zona sul, limpeza, primeiro emprego', 'Ex.: vagas zona sul, limpeza, free lances')
    .replace(
      /<label>UF<input name="state" maxlength="2" value="([^"]*)" required>([\s\S]*?)<\/label><label>Cidade<input name="city" value="([^"]*)" required>([\s\S]*?)<\/label>/,
      (_match, state, stateError, city, cityError) => `<label>Estado<select name="state" autocomplete="address-level1" data-location-state required>${publicStateOptions(decodeHtmlAttribute(state))}</select>${stateError}</label><label>Cidade<select name="city" autocomplete="address-level2" data-location-city data-selected-city="${escapeHtml(decodeHtmlAttribute(city))}" required>${publicCityOptions(decodeHtmlAttribute(state), decodeHtmlAttribute(city))}</select><small class="location-help" data-location-status aria-live="polite">Carregando todos os municípios do estado…</small>${cityError}</label>`,
    )
    .replace(
      /<label>UF<input name="estado" maxlength="2" value="([^"]*)" required>([\s\S]*?)<\/label><label>Cidade<input name="cidade" value="([^"]*)" required>([\s\S]*?)<\/label>/,
      (_match, state, stateError, city, cityError) => `<label>Estado<select name="estado" autocomplete="address-level1" data-location-state required>${publicStateOptions(decodeHtmlAttribute(state))}</select>${stateError}</label><label>Cidade<select name="cidade" autocomplete="address-level2" data-location-city data-selected-city="${escapeHtml(decodeHtmlAttribute(city))}" required>${publicCityOptions(decodeHtmlAttribute(state), decodeHtmlAttribute(city))}</select><small class="location-help" data-location-status aria-live="polite">Carregando todos os municípios do estado…</small>${cityError}</label>`,
    )
    .replace(/<div class="form-alert error">/g, '<div class="form-alert error" role="alert">')
...
function landingPage(req, res) {
  ...
  <main id="conteudo">
    ...
    <section class="discovery-strip" aria-labelledby="descobrir-titulo">
      ...
      <div class="container trust-bar" aria-label="Capacidades da plataforma">
      <div class="trust-inner">
        <div class="trust-item"><strong>Atração conectada</strong><span>Vagas, comunidades e campanhas alimentam um único fluxo.</span></div>
        <div class="trust-item"><strong>Conversa contínua</strong><span>O WhatsApp reduz abandono sem esconder o contexto da equipe.</span></div>
        <div class="trust-item"><strong>Qualificação visível</strong><span>Critérios, documentos e pendências aparecem antes da entrevista.</span></div>
        <div class="trust-item"><strong>Operação mensurável</strong><span>Origem, etapas e resultados ajudam a decidir o próximo movimento.</span></div>
      </div>
    </div>

    <section id="solucoes" class="section">
      <div class="container">
        <div class="section-head"><h2>Um fluxo de contratação que sua equipe consegue enxergar.</h2><p>Da aquisição à entrevista, cada etapa reduz abandono, retrabalho e tempo sem resposta.</p></div>
        <div class="feature-grid">
          <article class="feature-card accent"><div class="feature-icon">↗</div><h3>Aquisição de candidatos</h3><p>Portal, busca, grupos e campanhas com origem rastreável.</p></article>
          <article class="feature-card"><div class="feature-icon">◎</div><h3>Jornada no WhatsApp</h3><p>O candidato avança por perguntas e orientações claras, no canal que já usa.</p></article>
...
function activePortalWhere(alias = 'v') {
  return `
    ...
    AND (${alias}.data_encerramento IS NULL OR ${alias}.data_encerramento >= CURRENT_DATE)
  `;
}

async function listVacancies({ query = '', state = '', city = '', modality = '', page = 1, pageSize = 12 }) {
  ...
  if (state) {
    values.push(state);
    clauses.push(`UPPER(v.estado) = UPPER($${values.length})`);
  }
  if (modality) {
...
async function vacanciesPage(req, res, next) {
  try {
    ...
    if (state) queryBase.set('estado', state);
    ...
    const cards = result.vacancies.length
      ...
      : `<div class="empty-state"><h3>Nenhuma vaga encontrada</h3><p>Tente remover algum filtro ou acompanhe as próximas oportunidades no grupo.</p><a class="btn btn-primary" href="/grupo-oficial" data-track="CTA_SEM_RESULTADO_GRUPO">Entrar no grupo de vagas</a></div>`;
          ...
          <div class="container">
            ...
            <form class="search-shell" action="/vagas" method="get" data-search-form>
              ...
              <select name="estado" aria-label="Filtrar por estado" data-location-state>
                ${publicStateOptions(state, { allowBlank: true })}
              </select>
        ...
        <section class="portal-main">
          <div class="container portal-layout">
            ...
            <aside class="portal-sidebar">
              <article class="sidebar-card primary"><h3>Receba novas vagas</h3><p>Entre no grupo e acompanhe oportunidades assim que forem divulgadas.</p><a class="btn btn-accent btn-block" href="/grupo-oficial" data-track="CTA_LISTA_GRUPO">Entrar no grupo</a></article>
...
async function vacancyDetailPage(req, res, next) {
  try {
    ...
    const groupButton = groupUrl
      ? `<a class="btn btn-ghost btn-block" href="/grupo-oficial" data-track="CTA_VAGA_GRUPO">Entrar no grupo de vagas</a>`
      : '';
    ...
    const content = `${portalHeader({ active: 'vagas' })}
      <main id="conteudo" class="directory-main">
        ...
        <section class="job-detail-main">
          <div class="container job-detail-layout">
            ...
            <aside class="apply-card">
              ...
              <p>${application.type === 'whatsapp' ? 'Abra o WhatsApp com a vaga já identificada. No computador, escaneie o QR Code para continuar no celular.' : application.type === 'whatsapp-external' ? 'A empresa recebe candidaturas diretamente pelo WhatsApp. Clique abaixo para iniciar a conversa.' : application.type === 'email' ? 'Envie sua candidatura pelo e-mail indicado pela empresa.' : 'Continue no canal de candidatura definido pela empresa.'}</p>
              <a class="btn btn-accent btn-block btn-lg" href="${escapeHtml(application.url)}" target="${['whatsapp','whatsapp-external','external'].includes(application.type) ? '_blank' : '_self'}" rel="noopener" data-track="CTA_LATERAL_CANDIDATURA">${escapeHtml(application.label)}</a>
        ...
        <div class="sticky-mobile-cta">
          <a class="btn btn-accent" href="${escapeHtml(application.url)}" target="${['whatsapp','whatsapp-external','external'].includes(application.type) ? '_blank' : '_self'}" rel="noopener" data-track="CTA_MOBILE_CANDIDATURA">Candidatar-se</a>
          ${groupUrl ? '<a class="btn btn-ghost" href="/grupo-oficial" data-track="CTA_MOBILE_GRUPO">Grupo</a>' : ''}
        </div>
      </main>
      ${portalFooter()}`;
...
const companyLeadSchema = z.object({
  ...
  estado: z.preprocess(
    (value) => String(value || DEFAULT_STATE).trim().toUpperCase(),
...
function companyLeadPage(req, res, { errors = [], values = {} } = {}) {
  ...
  const errorHtml = errors.length
    ? `<div class="success-box" role="alert" style="border-color:#f4aaaa;background:#fff1f1;color:#9f2424"><strong>Revise os campos:</strong><br>${errors.map(escapeHtml).join('<br>')}</div>`
    : '';
  const content = `${header({ light: true })}
    <main id="conteudo">
      <section class="lead-hero">
        <div class="container lead-hero-grid">
          <div><span class="eyebrow">Para empresas que precisam contratar</span><h1>Conte a demanda. Nós desenhamos o caminho até a entrevista.</h1><p>Compartilhe cargos, volume e prazo. A Gênesis avalia como conectar atração, conversa, qualificação e agenda à sua operação.</p></div>
      ...
      <section class="lead-main">
        <div class="container lead-layout">
          ...
          <form class="lead-form" method="post" action="/api/public/empresas/interesse" data-track-form="empresa">
            ${sent ? '<div class="success-box" role="status"><strong>Recebemos seu interesse.</strong><br>A equipe da Gênesis poderá entrar em contato para entender a necessidade.</div>' : ''}
            ${errorHtml}
            ...
            <div class="form-grid">
              ...
              <div class="location-picker-pair" data-location-picker><div class="field"><label for="estado">Estado</label><select id="estado" name="estado" autocomplete="address-level1" data-location-state>${publicStateOptions(values.estado || DEFAULT_STATE)}</select></div><div class="field"><label for="cidade">Cidade</label><select id="cidade" name="cidade" autocomplete="address-level2" data-location-city data-selected-city="${escapeHtml(values.cidade || '')}">${publicCityOptions(values.estado || DEFAULT_STATE, values.cidade || '')}</select><small class="location-help" data-location-status aria-live="polite">Carregando todos os municípios do estado…</small></div></div>
              ...
              <div class="field full"><button class="btn btn-primary btn-lg btn-block" type="submit" data-track="ENVIAR_LEAD_EMPRESA">Quero conversar com a Gênesis</button><p class="form-note">Ao enviar, você autoriza o contato da equipe sobre esta necessidade de recrutamento.</p></div>
            </div>
          </form>
        </div>
      </section>
...
function validWhatsAppGroupUrl(value) {
  try {
    ...
    if (url.protocol !== 'https:' || !['chat.whatsapp.com', 'www.chat.whatsapp.com'].includes(url.hostname.toLowerCase())) return '';
    const code = url.pathname.split('/').filter(Boolean)[0] || '';
    return /^[A-Za-z0-9_-]{10,120}$/.test(code) ? `https://chat.whatsapp.com/${code}` : '';
  } catch { return ''; }
}
...
const companyPortals = registerCompanyPortalRoutes({
  app,
  pool,
  config: {
    SITE_URL,
...
app.get('/health/live', (_req, res) => {
  res.json({ status: 'ok', portal: 'vagas-grupos', version: APP_VERSION });
});

app.get('/health', async (_req, res) => {
  try {
    await pool.query('SELECT 1');
    res.json({ status: 'ok', portal: 'genesis-ecosystem', version: APP_VERSION, banco: 'disponível' });
  } catch (error) {
    console.error('Health check do PostgreSQL:', error);
    res.status(503).json({
      status: 'erro',
      portal: 'genesis-ecosystem',
      version: APP_VERSION,
      banco: 'indisponível',
...
app.get('/grupo-oficial', async (_req, res) => {
  try {
    const url = await getGroupInviteUrl();
    if (!url) return res.redirect(302, '/grupos?erro=Grupo oficial temporariamente indisponível');
    return res.redirect(302, url);
  } catch (error) {
    console.error('[OFFICIAL_GROUP_REDIRECT]', error.message);
    return res.redirect(302, '/grupos?erro=Grupo oficial temporariamente indisponível');
  }
});
...
app.get('/api/public/vagas', async (req, res, next) => {
  try {
    const result = await listVacancies({
      ...
      state: String(req.query.estado || '').trim().slice(0, 2).toUpperCase(),
...
app.get('/robots.txt', (_req, res) => {
  res.type('text/plain').send(`User-agent: *\nAllow: /\nDisallow: /api/\nDisallow: /minha-conta\nDisallow: /entrar\nDisallow: /cadastro\nDisallow: /meu-portal\nDisallow: /portal-para-empresas/comecar\nSitemap: ${SITE_URL}/sitemap.xml\n`);
});
...
app.get('/sitemap.xml', (_req, res) => {
  const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${[
    ...
    sitemapIndexEntry(`${SITE_URL}/sitemap-empresas.xml`),
  ].join('\n')}\n</sitemapindex>`;
...
app.get('/sitemap-empresas.xml', async (_req, res, next) => {
  try {
    const companies = await companyPortals.sitemapCompanies();
    const entries = companies.map((company) => ({
      loc: `${SITE_URL}/empresa/${company.slug}`,
      lastmod: isoDate(company.updated_at || company.publicado_em),
      priority: '0.8',
      changefreq: 'weekly',
    }));
    res.type('application/xml').send(`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${entries.map(sitemapUrlEntry).join('\n')}\n</urlset>`);
...
function start() {
  httpServer = app.listen(PORT, '0.0.0.0', () => {
    console.log(`Ecossistema Gênesis V${APP_VERSION} iniciado em 0.0.0.0:${PORT}.`);
