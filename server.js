...
function ageRequirementText(vacancy = {}) {
  const minimum = Number(vacancy.idade_minima || 0);
  const maximum = vacancy.idade_maxima === null || vacancy.idade_maxima === undefined || vacancy.idade_maxima === '' ? null : Number(vacancy.idade_maxima);
  if (minimum > 0 && Number.isFinite(maximum) && maximum >= minimum) return `Faixa etária: ${minimum} a ${maximum} anos`;
  if (minimum > 0) return `Idade mínima: ${minimum} anos`;
  if (Number.isFinite(maximum) && maximum > 0) return `Idade máxima: ${maximum} anos`;
  return '';
}
...
<head>
  ...
  <script src="/theme-init.js?v=600"></script>
  <link rel="stylesheet" href="/portal.css?v=600">
  ${schemas}
</head>
<body class="${escapeHtml(bodyClass)}">
  <a class="skip-link" href="#conteudo">Ir para o conteúdo</a>
  ${content}
  <script src="/portal.js?v=600" defer></script>
</body>
</html>`;
}
  ...
  return `<header class="site-header">
    <div class="container header-inner">
      <a class="brand" href="/" aria-label="Página inicial da ${escapeHtml(BRAND_NAME)}">
        ...
        <span>${escapeHtml(BRAND_NAME)}<small>Recruiting OS</small></span>
      </a>
  ...
  <main id="conteudo">
    ...
    <div class="container trust-bar">
      <div class="trust-inner">
        ...
        <div class="trust-item"><strong>Funil rastreável</strong><span>Origem, cliques, interesse e evolução do candidato.</span></div>
      </div>
    </div>
    ...
    <section id="solucoes" class="section">
      <div class="container">
        ...
        <div class="feature-grid">
          ...
          <article class="feature-card"><div class="feature-icon">◎</div><h3>Triagem no WhatsApp</h3><p>A Evelyn conduz o candidato, coleta informações e mantém a conversa avançando sem parecer um formulário frio.</p></article>
          ...
          <article class="feature-card"><div class="feature-icon">⌁</div><h3>Observabilidade real</h3><p>Indicadores, erros, mensagens, funil e desempenho por vaga para o administrador acompanhar.</p></article>
        </div>
      </div>
    </section>
    ...
    <section class="section section-dark">
      <div class="container split-panel">
        <article class="candidate-panel">
          ...
          <h3>Menos formulários. Mais clareza.</h3>
  ...
  return `<a class="job-card" href="/vagas/${escapeHtml(vacancySlug(vacancy))}" data-track="ABRIR_VAGA" data-track-label="${escapeHtml(vacancy.titulo)}">
    ...
    <div>
      ...
      <div class="job-meta">
        ...
        ${ageRequirementText(vacancy) ? `<span class="meta-chip">${escapeHtml(ageRequirementText(vacancy))}</span>` : ''}
...
function jobDescriptionHtml(vacancy) {
  ...
  if (ageRequirementText(vacancy)) requirements.unshift(ageRequirementText(vacancy));
...
async function vacancyDetailPage(req, res, next) {
  try {
    ...
    if (ageRequirementText(vacancy)) required.unshift(ageRequirementText(vacancy));
            ...
            <div class="job-content">
              <article class="content-card">
                ...
                <div class="detail-grid">
                  ...
                  <div class="detail-item"><span>Faixa etária</span><strong>${escapeHtml(ageRequirementText(vacancy) || 'Não definida')}</strong></div>
                </div>
              </article>
...
app.get('/health/live', (_req, res) => {
  res.json({ status: 'ok', portal: 'genesis-ia-v5.1.0' });
});

app.get('/health', async (_req, res) => {
  try {
    await pool.query('SELECT 1');
    res.json({ status: 'ok', portal: 'genesis-ia-v5.1.0', banco: 'disponível' });
  } catch (error) {
    console.error('Health check do PostgreSQL:', error);
    res.status(503).json({
      status: 'erro',
      portal: 'genesis-ia-v5.1.0',
      banco: 'indisponível',
...
function start() {
  httpServer = app.listen(PORT, '0.0.0.0', () => {
    console.log(`Portal público Genesis IA V5.1.0 iniciado em 0.0.0.0:${PORT}.`);
