...
function metaPage({ title, description, canonical, image, bodyClass = '', content, nonce, structuredData = [], robots = 'index,follow,max-image-preview:large' }) {
  ...
  return `<!doctype html>
<html lang="pt-BR" data-page="public">
<head>
  <meta charset="utf-8">
  ...
  <link rel="stylesheet" href="/portal.css?v=500">
  ${schemas}
</head>
<body class="${escapeHtml(bodyClass)}">
  <a class="skip-link" href="#conteudo">Ir para o conteúdo</a>
  ${content}
  <script src="/portal.js?v=500" defer></script>
</body>
</html>`;
}
  ...
  return `<header class="site-header">
    <div class="container header-inner">
      ...
      <div class="header-actions">
        <a class="btn btn-ghost" href="/vagas" data-track="CTA_HEADER_VAGAS">Ver vagas</a>
        <a class="btn btn-primary" href="${escapeHtml(loginUrl)}" data-track="CTA_HEADER_LOGIN">Login</a>
      </div>
    </div>
  </header>`;
}
        ...
        <section class="job-detail-main">
          <div class="container job-detail-layout">
            <div class="job-content">
              ...
              <article class="content-card"><h2>Benefícios</h2>${listBlock(benefits)}</article>
              ...
              ${vacancy.possui_insalubridade ? `<article class="content-card insalubrity-card"><h2>Adicional de insalubridade</h2><p><strong>${escapeHtml(vacancy.percentual_insalubridade || '')}% adicional</strong>${vacancy.observacao_insalubridade ? ` — ${escapeHtml(vacancy.observacao_insalubridade)}` : ''}.</p></article>` : ''}
...
app.get('/health/live', (_req, res) => {
  res.json({ status: 'ok', portal: 'genesis-ia-v5.0.1' });
});

app.get('/health', async (_req, res) => {
  try {
    await pool.query('SELECT 1');
    res.json({ status: 'ok', portal: 'genesis-ia-v5.0.1', banco: 'disponível' });
  } catch (error) {
    console.error('Health check do PostgreSQL:', error);
    res.status(503).json({
      status: 'erro',
      portal: 'genesis-ia-v5.0.1',
      banco: 'indisponível',
...
function start() {
  httpServer = app.listen(PORT, '0.0.0.0', () => {
    console.log(`Portal público Genesis IA V5.0.1 iniciado em 0.0.0.0:${PORT}.`);
