...
function registerCompanyPortalRoutes({ app, pool, config, helpers }) {
  const {
    ...
    AUTH_SECRET,
    SESSION_DAYS = 14,
    PORTAL_BRAND_NAME = 'Vagas & Grupos',
  ...
  async function requireCompanyAccount(req, res, next) {
    try {
      ...
      if (account.tipo !== 'EMPRESA') return res.status(403).send(simpleMessagePage(res, 'Área para empresas', 'Este espaço é destinado às contas do tipo Empresa.', '/minha-conta', 'Voltar para minha conta'));
      return next();
  ...
  function companyBrandStyle(portal) {
    const color = /^#[0-9a-f]{6}$/i.test(String(portal?.cor_primaria || '')) ? portal.cor_primaria : '#159c50';
  ...
  function landingPage(req, res) {
    ...
    const startHref = `/portal-para-empresas/comecar${ref ? `?ref=${encodeURIComponent(ref)}` : ''}`;
    const content = `${portalHeader({ active: '' })}<main id="conteudo" class="company-landing">
      <section class="company-lp-hero"><div class="directory-container company-lp-hero-grid"><div class="company-lp-copy"><span class="company-kicker">Portal de carreiras gratuito</span><h1>Seu Trabalhe Conosco, com a <em>marca da sua empresa.</em></h1><p>Centralize suas vagas em uma página profissional, compartilhe um único link e acompanhe o interesse dos candidatos sem precisar criar ou manter um site.</p><div class="company-lp-actions"><a class="btn btn-primary btn-lg" href="${escapeHtml(startHref)}" data-track="CTA_PORTAL_EMPRESA_COMECAR">Criar meu portal grátis</a><a class="btn btn-ghost btn-lg" href="#exemplo">Ver como fica</a></div><div class="company-lp-trust"><span>✓ Sua logomarca</span><span>✓ Vagas em um só lugar</span><span>✓ Métricas básicas</span><span>✓ Sem cartão</span></div></div><div id="exemplo" class="career-preview-shell" aria-label="Exemplo de portal de carreiras"><div class="career-preview-browser"><span></span><span></span><span></span><b>vagasegrupos.com.br/empresa/sua-empresa</b></div><div class="career-preview-cover"><div class="career-preview-logo">S</div><div><small>TRABALHE CONOSCO</small><h2>Sua Empresa</h2><p>Oportunidades para quem quer crescer junto.</p></div></div><div class="career-preview-body"><div><b>Vagas abertas</b><small>Encontre a oportunidade ideal</small></div><article><span>Auxiliar de Operações</span><small>São Paulo · Presencial</small><b>Ver vaga →</b></article><article><span>Assistente Administrativo</span><small>São Paulo · Híbrido</small><b>Ver vaga →</b></article><footer>Empresa parceira do Vagas & Grupos · Tecnologia Gênesis IA</footer></div></div></div></section>
      ...
      <section class="company-final-cta"><div class="directory-container"><div><h2>Crie seu portal de carreiras.</h2><p>Comece com a sua marca e publique a primeira vaga quando estiver pronto.</p></div><a class="btn btn-primary btn-lg" href="${escapeHtml(startHref)}">Criar meu portal grátis</a></div></section>
    </main>${portalFooter()}`;
    return res.send(metaPage({ title: 'Portal de carreiras gratuito para empresas', description: 'Crie uma página de carreiras com a marca da sua empresa, publique vagas e acompanhe visualizações e cliques.', canonical: `${SITE_URL}/portal-para-empresas`, image: `${SITE_URL}/assets/vagas-grupos-social.png`, bodyClass: 'light-page directory-site company-portal-page', nonce: res.locals.cspNonce, siteName: PORTAL_BRAND_NAME, titleSuffix: PORTAL_BRAND_NAME, favicon: '/assets/vagas-grupos-mark.svg', themeColor: '#159c50', structuredData: [], content }));
  }

  async function signupPage(req, res, { errors = {}, values = {} } = {}) {
    const account = await currentAccount(req);
    if (account?.tipo === 'EMPRESA') return res.redirect(302, '/meu-portal/onboarding');
  ...
  async function dashboardPage(req, res, next) {
    try {
      ...
      return res.send(metaPage({ title: 'Meu portal de carreiras', description: 'Gerencie a página de carreiras e as vagas da sua empresa.', canonical: `${SITE_URL}/meu-portal`, image: `${SITE_URL}/assets/vagas-grupos-social.png`, bodyClass: 'light-page directory-site company-portal-page career-dashboard-page', nonce: res.locals.cspNonce, robots: 'noindex,nofollow', siteName: PORTAL_BRAND_NAME, titleSuffix: PORTAL_BRAND_NAME, favicon: '/assets/vagas-grupos-mark.svg', themeColor: portal.cor_primaria || '#159c50', structuredData: [], content }));
    } catch (error) { return next(error); }
  }
